...
function SyncModal(props) {
    ...
    return (
        <div className='modal bg-modal rounded-md text-center'>
            ...
            <div className="btn-client p-3 bg-secondary hover:bg-primary rounded-md cursor-pointer mt-2" onClick={() => { props.SyncWithClient(); }}>Máy khách(máy bạn đó)</div>
            hoặc
            <div className="logout p-3 bg-[red] rounded-md cursor-pointer" onClick={() => dispatch(setToken("   "))}>Đăng xuất</div>
        </div>
    )
}
